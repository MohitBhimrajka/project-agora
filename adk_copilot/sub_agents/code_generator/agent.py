...
1.  **Initial Analysis & Architecture Proposal:**
    ...
    *   Your first response MUST be a single JSON object with three keys:
        ...
        - `"mermaid_syntax"`: The exact Mermaid graph syntax (`graph TD; ...`) that visually represents your plan. **CRITICAL MERMAID SYNTAX RULE: Node labels in double quotes (e.g., `Node["Label Text"]`) MUST NOT contain special characters like parentheses `()` or brackets `[]`. Use simple text only. For example, instead of `GreetTool["greet_tool()"]`, you MUST use `GreetTool["greet_tool"]`. Instead of `UserInput["User provides (text)"]`, you MUST use `UserInput["User provides text"]`.**
