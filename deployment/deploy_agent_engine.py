...
def main():
    ...
    group.add_argument(
        "--create", action="store_true", help="Create and deploy a new agent."
    )
    group.add_argument("--delete", action="store_true", help="Delete a deployed agent.")
    parser.add_argument(
        "--resource_id",
        type=str,
        help="The full resource name of the agent to delete (required for --delete).",
    )
    ...
    if not all([project_id, location, staging_bucket]):
        print(
            "Error: GOOGLE_CLOUD_PROJECT, GOOGLE_CLOUD_LOCATION, and GOOGLE_CLOUD_STORAGE_BUCKET must be set.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if args.delete:
        try:
            ...
            print(
                f"Error: Agent with resource ID '{args.resource_id}' not found.",
                file=sys.stderr,
            )
    ...
    if args.create:
        ...
        try:
            ...
            print(
                f"Build failed: No wheel file found in '{dist_dir}'.", file=sys.stderr
            )
            sys.exit(1)
        ...
        if not all(
            [
                env_vars["RAG_CORPUS_NAME"],
                env_vars["BQ_PROJECT_ID"],
                env_vars["BQ_DATASET_ID"],
            ]
        ):
            print(
                "Error: RAG_CORPUS_NAME, BQ_PROJECT_ID, and BQ_DATASET_ID must be set in .env before deployment.",
                file=sys.stderr,
            )
            sys.exit(1)
        ...
        print(
            "Deploying agent using agent_engines.create... This may take several minutes."
        )
